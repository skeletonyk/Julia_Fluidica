...
function velocity(w :: Array{Float64,2}, w_buffer :: Array{Float64,2}, s_buffer :: Array{Float64,2}, time :: Float64, lgfhat :: Array{Complex64,2},
    ...
    FFT_big :: FFTW_Type, IFFT_big :: IFFTW_Type)
       ...
       ufr :: Array{Float64,2} = frame_rot(time)* wcrossx_x + frame_xvel(time)#*ones(size(wcrossx_x))
       vfr :: Array{Float64,2}  = frame_rot(time)* wcrossx_y + frame_yvel(time)#*ones(size(wcrossx_y))
...
function avg_wx(w :: Array{Float64,2})
    ...
    [wx[i,j+1] = 0.5 * (w[i,j] + w[i,j+1]) for i = 1:size(w,1), j=1:size(w,2)-1 ]
    wx[:,1] = 0.5 * w[:,1]#[wx[i,1] = 0.5 * (w[i,1]) for i = 1:size(w,1) ]
    wx[:, n+1] = 0.5 * (w[:,n])#[wx[i, n+1] = 0.5 * (w[i,n]) for i = 1:size(w,1) ]
...
function avg_wy(w :: Array{Float64,2})
    ...
    [wy[i+1, j] = 0.5 * (w[i,j] + w[i+1,j]) for i = 1:size(w,1)-1, j=1:size(w,2)]
    wy[1,:] = 0.5 * w[1,:]# [wy[1  , j] = 0.5 * (w[1,j]) for j = 1:n ]
    wy[m+1,:] = 0.5 * w[m,:]#[wy[m+1, j] = 0.5 * (w[m,n]) for j = 1:n ]
    return wy
end
...
# velocity [comp=1,2,3 for u, v, or speed]  (frame="l','a','r" for lab, accelerating, or relative vel.).
    #println("test")
    #println(soln.v)
    u,v = velocity(soln.w,w_buffer,soln.t,lgfhat,
